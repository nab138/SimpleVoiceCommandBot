...
client.guildPlayers = guildPlayers;
...
async function executeCommand(command, args, message, isAudio){
    if(client.commands.has(command)){
        client.commands.get(command).execute(message, client, args, isAudio)
    } else if(isAudio) {
        ...
        if(client.guildPlayers.has(message.guild.id)){
            ...
            tts('Unknown Command', message.guild, function(){
            ...
            tts('Unknown Command', message.guild)
        }
    }
}
async function listen(receiver, message, commandMode){
    if(!commandMode){
        ...
        if(WakeWord.includes(text.toLowerCase())){
            tts('Listening for commands', message.guild)
